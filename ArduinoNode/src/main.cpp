...
const float BATTERY_SCALE = 3.0 * 1.3;
...
void read_sensors(int &temperature, int &humidity, int &soil, int &battery) {
    ...
    float battery_voltage = (bat_raw / 1023.0) * ADC_REF_VOLTAGE * BATTERY_SCALE;
    int bat_percent = (int)(((battery_voltage - BATTERY_EMPTY_VOLTAGE) / (BATTERY_FULL_VOLTAGE - BATTERY_EMPTY_VOLTAGE)) * 100.0);
    battery = clamp_int(bat_percent, 0, 99);
